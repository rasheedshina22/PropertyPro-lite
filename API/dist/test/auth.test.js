...
describe('Auth Route Endpoints', function () {
  ...
  describe('POST api/v1/auth/signin', function () {
    it('should successfully login a user if all required inputs are provided', function (done) {
      (0, _supertest["default"])(_app["default"]).post('/api/v1/auth/signin').send(_randomData.validLoginData).set('Accept', 'application/json').expect('Content-Type', /json/).expect(200).expect(function (res) {
    ...
    it('should not login a user if any of the input paramters is/are invalid', function (done) {
      (0, _supertest["default"])(_app["default"]).post('/api/v1/auth/signin').send(_randomData.invalidLoginData).set('Accept', 'application/json').expect('Content-Type', /json/).expect(401).expect(function (res) {
        var status = res.body.status;
        (0, _chai.expect)(status).to.equal('401 Unauthorized');
        (0, _chai.expect)(res.body).to.have.all.keys('status', 'error');
      }).end(done);
